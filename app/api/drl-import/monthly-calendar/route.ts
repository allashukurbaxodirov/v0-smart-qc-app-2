...
// GET /api/drl-import/monthly-calendar?year=2026&month=5
...
// DELETE — faqat DRL import ma'lumotlarini o'chirish, kalendarni saqlab qolish
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    for (const b of batches) {
      await sql`DELETE FROM drl_imports WHERE import_batch = ${b.id}::uuid`
      await sql`DELETE FROM drl_import_batches WHERE import_batch = ${b.id}::uuid`
      deleted++
    }
