...
import { parseGsipDRLBySmena } from '@/lib/gsip-drl-parser'
...
// ─── POST: Oylik DRL hisobot — smena jadvali bilan import ─────────────────────
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { meta, bySmena, warnings, totalRaw, totalSkipped } =
      parseGsipDRLBySmena(buffer, calendar)
    ...
    try { await sql`ALTER TABLE drl_import_batches ADD COLUMN IF NOT EXISTS shift_label TEXT` } catch {}
    try { await sql`ALTER TABLE drl_imports ADD COLUMN IF NOT EXISTS defect_date DATE` } catch {}

    const results: Record<string, {
      importBatch: string; rowCount: number; rawCount: number; totalCount: number
    }> = {}
      ...
      for (let i = 0; i < rows.length; i += CHUNK) {
        ...
        await sql`
          INSERT INTO drl_imports ${sql(chunk.map(r => ({
            ...
            veh_cnt:      r.vehCnt  ?? 0,
            defect_date:  r.defectDate || null,
          })))}
        `
      }

      results[smena] = { importBatch, rowCount: rows.length, rawCount, totalCount }
    }

    return NextResponse.json({ ok: true, meta, results, warnings, totalRaw, totalSkipped })
  } catch (e: any) {
    console.error('drl-import/monthly POST error:', e.message)
