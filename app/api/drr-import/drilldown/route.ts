...
/**
 * GET /api/drr-import/drilldown?batch=UUID&lv1=Doors+%2F+Gates
 *
 * Returns flat rows for a given part_lv1, grouped for client-side tree building:
 * lv2 → lv3 → lv4 → [ fault rows with defect_note ]
 */
  ...
  try {
    const rows = await sql`
      SELECT
        ...
        fault_code,
        fault_name,
        COALESCE(defect_note, '') AS defect_note,
        shop,
        prod_team,
        model_group,
        SUM(count)::int           AS total_count,
        SUM(veh_cnt)::int         AS total_veh,
      ...
      GROUP BY
        ...
        fault_code, fault_name, defect_note,
    ...
    console.error('drr drilldown error:', e.message)
