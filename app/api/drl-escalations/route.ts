...
// ?role=welding_engineer  → filter by assigned role (for engineer dashboards)
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    console.error('drl-escalations GET error:', e.message)
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.error('drl-escalations POST error:', e.message)
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    ...
    console.error('drl-escalations PATCH error:', e.message)
    return NextResponse.json({ error: e.message }, { status: 500 })
  }
}
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    await sql`DELETE FROM drl_escalations WHERE id = ${id}::uuid`
    return NextResponse.json({ ok: true })
  } catch (e: any) {
    return NextResponse.json({ error: e.message }, { status: 500 })
  }
