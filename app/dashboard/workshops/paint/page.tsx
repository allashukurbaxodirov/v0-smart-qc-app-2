...
import { paintSectors, paintSectorDetails } from '@/lib/mock-data'
import { ChevronLeft } from 'lucide-react'
import { Button } from '@/components/ui/button'
import { Badge } from '@/components/ui/badge'
import Link from 'next/link'

export default function PaintDrilldownPage() {
  const [activeShift, setActiveShift] = useState('A')
  const [selectedSector, setSelectedSector] = useState<string | null>(null)

  const shopData = paintSectors['PAINT SHOP']
  const shiftData = shopData?.[activeShift as keyof typeof shopData] || []
  const sectorDetails = selectedSector ? paintSectorDetails[selectedSector as keyof typeof paintSectorDetails] : null
              ...
              >
                ...
                <div className="flex items-start justify-between mb-4">
                  ...
                  <Badge className={getStatusBadge(sector.status)}>
                    {sector.status === 'good' ? '✓' : sector.status === 'warning' ? '⚠' : '✕'}
                  </Badge>
                </div>
