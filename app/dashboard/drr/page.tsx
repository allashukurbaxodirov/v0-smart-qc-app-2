      ...
      {escModal && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-card border border-border rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden">
            {/* Gradient header */}
            <div className="bg-gradient-to-r from-orange-600/20 to-orange-500/5 border-b border-orange-500/20 px-6 py-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-xl bg-orange-500/20 border border-orange-500/40 flex items-center justify-center">
                  <Bell className="w-4 h-4 text-orange-400" />
                </div>
                <div>
                  <h3 className="font-bold text-foreground text-sm">DRR Eskalatsiya yaratish</h3>
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="bg-orange-500/5 border border-orange-500/20 rounded-xl p-3.5">
                <div className="flex items-center gap-2 flex-wrap mb-1.5">
                  {escModal.fault.fault_code !== '—' && (
                    <span className="text-xs font-mono bg-orange-500/15 text-orange-400 px-2 py-0.5 rounded-md font-bold">
                      {escModal.fault.fault_code}
                    </span>
                  )}
                ...
                <div className="flex flex-wrap gap-3 text-xs">
                  <span className="text-muted-foreground">Nuqson: <strong className="text-foreground">{escModal.fault.total_count}</strong></span>
                  <span className="text-muted-foreground">Mashina: <strong className="text-orange-400">{escModal.fault.total_veh_cnt}</strong></span>
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-4 gap-2">
                  {[
                    { value: 'ga_engineer',      label: 'GA',      sub: 'GA Muhandis',      activeCls: 'border-emerald-500 bg-emerald-500/10',   textCls: 'text-emerald-400',  dotCls: 'bg-emerald-500' },
                    { value: 'welding_engineer', label: 'Welding', sub: 'Welding Muh.',     activeCls: 'border-sky-500 bg-sky-500/10',           textCls: 'text-sky-400',      dotCls: 'bg-sky-500' },
                    { value: 'press_engineer',   label: 'Press',   sub: 'Press Shop',       activeCls: 'border-amber-500 bg-amber-500/10',       textCls: 'text-amber-400',    dotCls: 'bg-amber-500' },
                    { value: 'paint_engineer',   label: 'Paint',   sub: 'Paint Shop',       activeCls: 'border-violet-500 bg-violet-500/10',     textCls: 'text-violet-400',   dotCls: 'bg-violet-500' },
                    { value: 'sqe_engineer',     label: 'SQE',     sub: 'SQE Muhandis',     activeCls: 'border-pink-500 bg-pink-500/10',         textCls: 'text-pink-400',     dotCls: 'bg-pink-500' },
                    { value: 'qe_engineer',      label: 'QE',      sub: 'QE Muhandis',      activeCls: 'border-cyan-500 bg-cyan-500/10',         textCls: 'text-cyan-400',     dotCls: 'bg-cyan-500' },
                    { value: 'pe_engineer',      label: 'PE',      sub: 'PE Muhandis',      activeCls: 'border-lime-500 bg-lime-500/10',         textCls: 'text-lime-400',     dotCls: 'bg-lime-500' },
                  ].map(r => {
                    ...
                    return (
                      <button key={r.value} type="button"
                        ...
                        <span className={`text-xs font-bold leading-tight ${isSelected ? '' : ''}`}>{r.label}</span>
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  rows={2}
                  className="w-full px-3 py-2 bg-background border border-border rounded-xl text-foreground text-sm resize-none focus:border-orange-500/50 focus:outline-none transition-colors"
                />
              </div>
              ...
              <div className="flex gap-2 pt-1">
                ...
                <button onClick={submitEscalation} disabled={escSaving}
                  className="flex-1 py-2.5 rounded-xl bg-orange-600 text-white text-sm font-bold hover:bg-orange-700 disabled:opacity-50 transition-all flex items-center justify-center gap-2">
                  {escSaving ? (
