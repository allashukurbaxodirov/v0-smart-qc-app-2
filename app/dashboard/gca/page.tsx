...
import {
  ...
  BookOpen, Upload, CheckCircle2, ChevronRight,
} from 'lucide-react'
...
export default function GCAPage() {
  ...
  return (
    <div className="min-h-screen bg-background">
      {monthlyOpen && <GCAMonthlyModal onClose={() => setMonthlyOpen(false)} />}
      <PageHeader
        title="GCA Dashboard"
...
// ═══════════════════════════════════════════════════════════════════════════════
// OYLIK GCA HISOBOT MODAL
// ═══════════════════════════════════════════════════════════════════════════════

const MONTHS_UZ_GCA = ['Yanvar','Fevral','Mart','Aprel','May','Iyun','Iyul','Avgust','Sentabr','Oktabr','Noyabr','Dekabr']
const SMENA_OPTS_GCA = ['A', 'B', 'D', '—'] as const
type SmenaOptGCA = 'A' | 'B' | 'D' | '—'
type DayCalendarGCA = { E: SmenaOptGCA; N: SmenaOptGCA }

interface GCAMonthlyResult {
  ok: boolean
  meta: { dateFrom: string; dateTo: string }
  results: Record<string, { importBatch: string; rowCount: number; rawCount: number; totalWeight: number; vehCount: number }>
  warnings: string[]
  totalRaw: number
  totalSkipped: number
}

function buildDaysGCA(year: number, month: number): string[] {
...
function autoFillGCA(days: string[], startSmena: SmenaOptGCA): Record<string, DayCalendarGCA> {
  const rota: SmenaOptGCA[] = ['A', 'B', 'D']
  const idx = rota.indexOf(startSmena as any)
  const start = idx < 0 ? 0 : idx
  const cal: Record<string, DayCalendarGCA> = {}
  days.forEach((date, i) => {
    cal[date] = {
      E: rota[(start + i * 2)     % 3],
...
function GCAMonthlyModal({ onClose }: { onClose: () => void }) {
  ...
  const days = buildDaysGCA(year, month)

  const [calendar,  setCalendar]  = useState<Record<string, DayCalendarGCA>>(() =>
    Object.fromEntries(days.map(d => [d, { E: 'A' as SmenaOptGCA, N: 'B' as SmenaOptGCA }]))
  )
  ...
  useEffect(() => {
    const newDays = buildDaysGCA(year, month)
    setCalendar(Object.fromEntries(newDays.map(d => [d, { E: 'A' as SmenaOptGCA, N: 'B' as SmenaOptGCA }])))
    setCalSaved(false); setDeleteMsg('')
    fetch(`/api/gca-import/monthly-calendar?year=${year}&month=${month}`)
      .then(r => r.ok ? r.json() : null)
      .then(data => {
        if (data?.found && data.calendar) {
          const loaded: Record<string, DayCalendarGCA> = {}
          for (const date of newDays) {
            const saved = data.calendar[date]
            loaded[date] = { E: (saved?.E as SmenaOptGCA) ?? 'A', N: (saved?.N as SmenaOptGCA) ?? 'B' }
          }
  ...
  const setDay = (date: string, slot: 'E' | 'N', val: SmenaOptGCA) =>
    setCalendar(p => ({ ...p, [date]: { ...p[date], [slot]: val } }))

  const applyAutoFill = (start: SmenaOptGCA) =>
    setCalendar(autoFillGCA(buildDaysGCA(year, month), start))

  const [file,      setFile]      = useState<File | null>(null)
  const [uploading, setUploading] = useState(false)
  const [result,    setResult]    = useState<GCAMonthlyResult | null>(null)
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const res  = await fetch('/api/gca-import/monthly', { method: 'POST', body: fd })
      const data = await res.json()
      ...
      <div className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-bold text-foreground">Oylik GCA Hisobot</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          {step === 1 && (
            <div className="space-y-6">
              <p className="text-sm text-muted-foreground">Qaysi oyning GCA hisobotini kiritmoqchisiz?</p>
              <div className="flex items-center gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <select value={month} onChange={e => setMonth(Number(e.target.value))}
                    className="px-4 py-2.5 bg-muted/30 border border-border rounded-xl text-foreground text-sm font-semibold focus:outline-none">
                    {MONTHS_UZ_GCA.map((m, i) => <option key={i+1} value={i+1}>{m}</option>)}
                  </select>
                </div>
              </div>
              <div className="p-4 bg-indigo-500/5 border border-indigo-500/20 rounded-xl">
                <p className="text-sm font-semibold text-indigo-400 mb-1">{MONTHS_UZ_GCA[month-1]} {year} — {days.length} kun</p>
                <p className="text-xs text-muted-foreground">
                  GCA yozuvlari har kun kunduz (E) smenasiga ko&apos;ra A/B/D ga ajratiladi
                </p>
              </div>
              {calSaved && (
                <div className="flex items-center gap-2 px-3 py-2 bg-emerald-500/10 border border-emerald-500/30 rounded-xl">
                  <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" />
                  <p className="text-xs font-semibold text-emerald-400">{MONTHS_UZ_GCA[month-1]} {year} uchun smena jadvali DB da saqlangan — avtomatik yuklandi</p>
                </div>
              )}
            </div>
          )}
          ...
          {step === 2 && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <p className="text-sm font-semibold text-foreground">{MONTHS_UZ_GCA[month-1]} {year} — Smena jadvali</p>
                <div className="flex items-center gap-2">
                  <span className="text-xs text-muted-foreground">Avtoto&apos;ldirish:</span>
                  {(['A','B','D'] as SmenaOptGCA[]).map(s => (
                    ...
                    return (
                      <div key={date} className={`grid grid-cols-[auto_1fr_1fr] items-center ${isWkd ? 'bg-amber-500/5' : ''}`}>
                        <div className="px-4 py-2 min-w-[72px]">
                          <span className={`text-sm font-bold ${isWkd ? 'text-amber-400' : 'text-foreground'}`}>{d}-{MONTHS_UZ_GCA[month-1].toLowerCase().slice(0,3)}</span>
          ...
          {step === 3 && (
            <div className="space-y-5">
              <div className="p-4 bg-indigo-500/5 border border-indigo-500/20 rounded-xl text-xs text-muted-foreground">
                <p className="font-semibold text-indigo-400 mb-1">{MONTHS_UZ_GCA[month-1]} {year} — Smena jadvali tayyorlandi ✓</p>
                <p>Endi GSIP GCA Defect Details Excel faylini yuklang. Har kun kunduz smenasi (E) bo&apos;yicha A/B/D ga ajratiladi.</p>
              </div>
              {!result && (
                <label className={`flex flex-col items-center justify-center gap-3 p-8 border-2 border-dashed rounded-2xl cursor-pointer transition-all ${file?'border-indigo-500/60 bg-indigo-500/5':'border-border hover:border-indigo-500/40 hover:bg-indigo-500/5'}`}>
                  ...
                  {file ? (
                    ...
                    <><p className="text-sm font-semibold text-foreground">Excel faylni tanlang yoki tashlang</p><p className="text-xs text-muted-foreground">GSIP GCA Defect Details — .xlsx format</p></>
                  )}
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {Object.entries(result.results).map(([smena, r]) => (
                      <div key={smena} className={`rounded-xl border p-4 ${smena==='A'?'border-blue-500/30 bg-blue-500/5':smena==='B'?'border-green-500/30 bg-green-500/5':'border-purple-500/30 bg-purple-500/5'}`}>
                        ...
                        <p className="text-2xl font-bold text-foreground">{r.totalWeight.toLocaleString()}</p>
                        <p className="text-xs text-muted-foreground mt-0.5">og&apos;irlik balli · {r.vehCount} mashina</p>
                  ...
                  <div className="pt-2 border-t border-border">
                    ...
                    <button onClick={async () => {
                      if (!confirm(`${MONTHS_UZ_GCA[month-1]} ${year} GCA import ma'lumotlarini o'chirasizmi?`)) return
                      setDeleting(true); setDeleteMsg('')
                      try {
                        const res = await fetch(`/api/gca-import/monthly-calendar?year=${year}&month=${month}`, { method: 'DELETE' })
                        const data = await res.json()
