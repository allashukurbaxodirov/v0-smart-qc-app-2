...
import { drlDefectsByShop } from '@/lib/mock-data'
...
export default function DRLPage() {
  ...
  const getTotalDefects = () => {
    return shops.reduce((sum, shop) => {
      return sum + (drlDefectsByShop[shop]?.reduce((s, d) => s + d.count, 0) || 0)
    }, 0)
  }
  ...
  const getShopDefects = (shop: string) => {
    const defects = drlDefectsByShop[shop as keyof typeof drlDefectsByShop] || []
  ...
  const getShopTotal = (shop: string) => {
    return drlDefectsByShop[shop as keyof typeof drlDefectsByShop]?.reduce((sum, d) => sum + d.count, 0) || 0
  }
  ...
  if (selectedShop) {
    ...
    return (
      <div className="min-h-screen bg-background">
        <PageHeader
          title={selectedShop}
          description="DRL nuqsonlari va xavflilik faktorlari"
          breadcrumbs={[
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* Page Header */}
      <PageHeader
        title="DRL aniqlangan muammolar"
        description="Detailed Review Level - Taqsimot bo'yicha nuqsonlar tahlili"
        breadcrumbs={[
          { label: 'Dashboard', href: '/dashboard' },
          { label: 'DRL' },
        ]}
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="bg-card border border-primary/30 rounded-xl p-8 bg-primary/5">
          <p className="text-sm text-muted-foreground mb-2">DRL aniqlangan muammolar umumiy soni</p>
        ...
        {/* Shop Distribution */}
        <div>
          <h2 className="text-xl font-bold text-foreground mb-4">Sexlar bo'yicha taqsimot</h2>
